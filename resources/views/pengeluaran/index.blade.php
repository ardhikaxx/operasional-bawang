            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ route('pengeluaran.show', $item->id) }}" class="btn btn-sm btn-outline-info me-1" title="Detail"><i class="fas fa-eye"></i></a>
                        @if($item->status === 'draft' || auth()->user()->role === 'owner')
                            <a href="{{ route('pengeluaran.edit', $item->id) }}" class="btn btn-sm btn-outline-warning me-1" title="Edit"><i class="fas fa-edit"></i></a>
                        @endif
                        @if(auth()->user()->role === 'owner')
                            @if($item->status === 'draft')
                            <button type="button" class="btn btn-sm btn-success me-1" onclick="verifikasiForm('ver-exp-{{ $item->id }}')" title="Verifikasi"><i class="fas fa-check"></i></button>
                            <form id="ver-exp-{{ $item->id }}" action="{{ route('pengeluaran.verifikasi', $item->id) }}" method="POST" class="d-none">
                                @csrf
                                @method('PATCH')
                            </form>
                            @endif
                            <button type="button" class="btn btn-sm btn-outline-danger" onclick="hapusForm('del-exp-{{ $item->id }}')" title="Hapus"><i class="fas fa-trash"></i></button>
